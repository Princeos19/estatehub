  ...
  if (success) {
    return (
      <div className="min-h-screen bg-brand-gray flex items-center justify-center px-4 pt-16">
        <div className="w-full max-w-sm text-center bg-white rounded-2xl shadow-card p-10">
          <div className="w-16 h-16 bg-brand-green/10 rounded-full flex items-center justify-center mx-auto mb-4">
            <span className="text-3xl">✉️</span>
          </div>
          <h2 className="text-2xl font-bold text-brand-text mb-2">Account created!</h2>
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-card p-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-brand-gray-3 rounded-xl text-sm font-medium text-brand-text hover:bg-brand-gray transition-colors mb-4 disabled:opacity-60"
          >
            {googleLoading ? (
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 h-px bg-brand-gray-3" />
            <span className="text-xs text-brand-text-3">or</span>
            <div className="flex-1 h-px bg-brand-gray-3" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input type="text" required autoFocus placeholder="Your full name" className="input-field" value={form.fullName} onChange={set('fullName')} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-brand-text-2 mb-1.5">Email address</label>
              <input type="email" required placeholder="you@email.com" className="input-field" value={form.email} onChange={set('email')} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-brand-text-2 mb-1.5">Password</label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required placeholder="Min. 6 characters" className="input-field pr-10" value={form.password} onChange={set('password')} />
            ...
            <div>
              ...
              <input type="password" required placeholder="Repeat your password" className="input-field" value={form.confirm} onChange={set('confirm')} />
            </div>
